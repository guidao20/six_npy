...
class FID(object):
        def __init__(self, epsilon, alpha, steps, T_niters, mu, gamma,  target_layer, bound, device):
                ...
                self.steps = steps # Denotes N
                self.T_niters = T_niters
                self.alpha = alpha 
                self.bound = bound
                self.device = device
                self.mu = mu
                self.gamma = gamma # 1 - drop_probality
        ...
        def attack(self, model, ori_img, label):
                ...
                for n in range(self.steps):
                        # mask_x = torch.bernoulli(torch.ones_like(ori_img.shape) * self.gamma)
                        x_mask = torch.bernoulli(torch.ones_like(ori_img) * self.gamma)
...
if __name__ == '__main__':
        ...
        attack_type = FID(epsilon, alpha, steps, T_niters, mu, gamma, target_layer, bound, device)
