...
parser.add_argument('--target_layer', type=str, default = ['layer1', 'layer2', 'layer3', 'layer4'])
        ...
        def attack(self, model, ori_img, label):
                ...
                for t in range(self.T_niters):
                        ...
                        for idx, layer in enumerate(self.target_layer):
                                ...
                                loss = torch.log(torch.sum(negative)) - torch.log(torch.sum(positive))
                                loss.backward()
                                grad += x_adv.grad
                                x_adv = x_adv.detach()
...
if __name__ == '__main__':
        ...
        attack_type = FDA(epsilon, alpha, steps, T_niters, mu, gamma, target_layer, bound, device)
        correct = 0
        label_ls = []
